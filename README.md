# Egypt's Energy Landscape: A Comparative Analysis

## Project Overview

This repository contains the final project for the **DEPI Power BI
Specialist Track**. The project provides a comparative, data‑driven
analysis of **Egypt's electric power generation** and **renewable energy
transition** against countries with similar generation profiles.\
The objective is to benchmark Egypt's performance, assess strengths and
weaknesses, and deliver actionable insights for policymakers and
energy‑sector stakeholders.

The analysis uses data from international sources and tools including
**Excel**, **Python**, and **Power BI**.

------------------------------------------------------------------------

## Team Members


  -  **Ahmed Abdelaty Tawfik Gohar**  
     - Student ID: 21093569  
     - Email: ahmedgohar11@gmail.com  
  

- **Amr Fawzy Abdelhamed Abdelaziz**  
  - Student ID: 21100957  
  - Email: amrfawzyabdelhamied@gmail.com  

- **Mahmoud Abdelnaby Refaat**  
  - Student ID: 21045900  
  - Email: mahmoudrefaataa@gmail.com  

- **Mahmoud Magdy Elsaeed Abdelrahman**  
  - Student ID: 21089136  
  - Email: mahmoudelsaaed4@gmail.com  


------------------------------------------------------------------------

## Project Methodology

1.  **Country Selection**\
    Identify countries with total electricity generation within ±10% of
    Egypt's level.

2.  **Generation Mix Analysis**\
    Compare electricity sources: fossil fuels, hydro, solar, wind, and
    nuclear.

3.  **Performance Benchmarking**\
    Assess key indicators:

    -   Carbon and GHG emissions
    -   Energy conversion efficiency
    -   Energy demand
    -   Renewable energy share and growth

4.  **Renewable Energy Deep Dive**\
    Analyze renewable potential, national strategies, and socio‑economic
    impacts.

5.  **Data Visualization & Insights**\
    Develop interactive Power BI dashboards for comparison and ranking.

6.  **Recommendations**\
    Provide evidence‑based recommendations for Egypt's energy
    transition.

------------------------------------------------------------------------

## Key Deliverables

-   **Stakeholder Analysis** -- A detailed mapping of Egypt's
    energy-sector stakeholders.
-   **Project Plan** -- A step-by-step methodology for executing the
    full analysis.
-   **Interactive Power BI Dashboard** -- Featuring:
    -   Generation mix comparisons
    -   Emission & efficiency metrics
    -   Renewable growth assessment
    -   Egypt's ranking vs. peer countries

------------------------------------------------------------------------

## Stakeholders

### **Primary Stakeholders (High Interest / High Influence)**

-   Ministry of Electricity and Renewable Energy
-   New and Renewable Energy Authority (NREA)
-   Egyptian Electricity Holding Company (EEHC)

### **Secondary Stakeholders (Medium Interest / Influence)**

-   Public & Private Energy Companies
-   Regional Center for Renewable Energy & Energy Efficiency (RCREEE)
-   International data organizations (IEA, World Bank, OWID)
-   Researchers and academic institutions

### **Tertiary Stakeholders (Low / Medium Influence)**

-   Environmental NGOs (Greenish, Banlastic Egypt)
-   General public and electricity consumers

------------------------------------------------------------------------

## Data Sources

-   International Energy Agency (IEA)
-   The World Bank
-   Our World in Data (OWID)
-   International Renewable Energy Agency (IRENA)

------------------------------------------------------------------------

## Tools & Technologies

-   **Microsoft Power BI** -- Visualization & dashboards
-   **Python** -- Data cleaning, processing, analysis
-   **Microsoft Excel** -- Pre-processing & data validation
-   **Git & GitHub** -- Version control & collaboration

------------------------------------------------------------------------

## Repository Structure

    ├── Data/                 # Raw and processed datasets
    ├── Documentation/        # Stakeholder analysis & project plan PDFs
    ├── Notebooks/            # Python notebooks used for analysis
    ├── PowerBI_Dashboards/   # Power BI files and exports
    ├── Reports/              # Final summary reports & presentations
    └── README.md             # This file

------------------------------------------------------------------------

## License

This project is created as part of the **DEPI Power BI Specialist
Track** and is intended for educational and analytical use.
